...
## ТВОЙ АЛГОРИТМ:
  ...
  3. После внесения изменений запусти тесты (например, pnpm typecheck и pnpm test), чтобы убедиться, что всё работает.
  4. Если тесты прошли:
  ...
  5. Если задача требует вмешательства человека или внешних доступов:
     - Установи blocked: true.
     - Опиши причину в notes.
     - Переходи к следующей задаче без коммита.
  6. Если нет задач с passes: false и blocked: false, выведи сообщение: <promise>COMPLETE</promise>.